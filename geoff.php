...
$testFile=$current_dir . DIRECTORY_SEPARATOR . $prefix . $now;
touch($testFile);
ini_set('track_errors', 1);
date_default_timezone_set('America/Phoenix');
$testFiles = array($testFile, $filePath);
// END CONFIG
...
function addFilestoArray(&$testFiles, $current_dir, $prefix) {
    $directory = new DirectoryIterator($current_dir);
...
<body>
  <header>
    <script>
        window.onbeforeunload = function() {
            navigator.sendBeacon('<? echo $filePath; ?>', '');
        };
    </script>
    <nav class="navbar navbar-expand-md navbar-dark fixed-top bg-dark">
      <span class="navbar-brand mb-0 h1">GoDaddy GEOFF Tool</span><br>
      <span class="navbar-brand mb-0 h1">Host: <?php echo $os; ?> </span>
      <form method="post">
  ...
  <script>
    function callRemoveSelf() {
      var currentFileName = "<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>";
      var xhr = new XMLHttpRequest();
      xhr.open("GET", currentFileName, true);
      xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest'); // Set custom header for AJAX identification
      xhr.send();
}
...
// Check if the remove button was clicked and call the removeFiles function
